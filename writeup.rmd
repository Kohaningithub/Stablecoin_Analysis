...
The solid red line shows actual peg deviation, while the dashed green line shows our model's predictions. Despite achieving strong statistical performance (Rsquare = 0.95, RMSE = 0.074467), the model's predictive accuracy varies across different phases of the collapse:
...
Taking USDC's tree as an illustrative example, we observe a clear three-tier classification structure based on peg deviation thresholds. The primary split occurs at a deviation of 50x10-6 (0.005%), effectively separating the stable state from potential instability. A secondary threshold at 150x10-6 (0.015%) further distinguishes between unstable and depegged states. The model achieves perfect classification accuracy (100%) using just these two thresholds, suggesting that peg deviation alone is sufficient for stability assessment.
...
The prediction performance metrics reveal interesting variations across tokens. USTC shows the highest Rsquare value (0.913) but also the largest RMSE (0.1308), reflecting its high-variance behavior during the crisis period, as evident in its prediction plot. USDT demonstrates moderate predictive success (Rsquare = 0.723, RMSE = 0.0002), with its predictions closely tracking actual values except during extreme events. In contrast, other collateralized stablecoins show very low Rsquare values (DAI: 0.013, PAX: 0.001, USDC: 0.003) but extremely small RMSEs (0.0003, 0.0018, 0.0001 respectively). These metrics reveal an important insight: while the models struggle to explain the variance in highly stable tokens (low Rsquare), their predictions remain extremely accurate in absolute terms (low RMSE). This pattern suggests that collateralized stablecoins maintain such tight peg control that their minor deviations are essentially random noise, making them statistically unpredictable but operationally reliable.
...
3. **Stability Paradox**: The most stable tokens showed the poorest regression performance (low Rsquare) despite having the best stability outcomes (low RMSE), suggesting well-functioning stablecoins maintain such tight peg control that their minor deviations are essentially random noise.
...
compare_models <- function(tree_results, forest_results, model_results) {
  ...
  p1 <- performance_data %>%
    ...
    labs(title = "Model Explanatory Power",
         subtitle = "Higher Rsquare indicates better variance explanation",
         x = "Stablecoin", 
         y = "R-squared",
         fill = "Model Type") +
    scale_fill_viridis_d() +
